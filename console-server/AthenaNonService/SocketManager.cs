...
namespace AthenaService
{
    class SocketManager
    {
        ...
        public bool KickConnection(int n)
        {
            ...
            {
                Program.doEventLog("SocketManager.KickConnection: " + e.Message + "\r\n" + e.StackTrace, 0);
            }
            return X;
        }
        ...
        private void looper()
        {
            try
            {
                ...
                while (doRun)
                {
                    SockReciever reciever = new SockReciever(mRecievers, sockSrv.AcceptTcpClient());
                    lock (mRecievers.SyncRoot)
                    {
        ...
        public ArrayList WhosOnline()
        {
            ...
            {
                Program.doEventLog("SocketManager.WhosOnline: " + e.Message + "\r\n" + e.StackTrace, 2);
            }
            return userList;
        }
